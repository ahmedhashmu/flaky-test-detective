...
def overview_payload(
    ...
    is_demo = DEMO_RUNNER in stats.runners
    ...
    return {
        ...
        "caveats": _caveats(report, score, is_demo=is_demo),
        "is_demo": is_demo,
    }
...
def _caveats(
    report: AnalysisReport, score: TrustScore, *, is_demo: bool = False
) -> list[dict[str, str]]:
    ...
    # First, and unmissable. Every verdict below was produced by the real detector, but
    # from generated history: these test executions did not happen. A demo mistaken for
    # real results would cost more credibility than the demo is worth.
    if is_demo:
        caveats.append(
            {
                "severity": "warning",
                "title": "Demo data",
                "detail": (
                    "This history was generated by `flaky demo`, not recorded from a real "
                    "suite. The verdicts are genuine output from the same analysis the CLI "
                    "runs, but the runs behind them are synthetic. Point --db at your own "
                    "database to see real results."
                ),
            }
        )
